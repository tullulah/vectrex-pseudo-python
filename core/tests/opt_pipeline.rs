...
fn constant_folding_add_mul_identities() {
    ...
    if let Item::Function(fun) = &opt.items[0] {
        ...
        match &fun.body[0] { Stmt::Return(Some(Expr::Number(5)), _) => {}, _ => panic!("expected Return(Number(5)) got {:?}", fun.body[0]) }
...
fn dead_store_elimination_basic() {
    ...
    let f = Function { name: "f".into(), params: vec![], body: vec![
        Stmt::Let { name: "x".into(), value: Expr::Number(1), source_line: 0 }, // dead
    ...
    if let Item::Function(fun) = &opt.items[0] {
        ...
        match &fun.body[0] { Stmt::Return(Some(Expr::Number(2)), _) => {}, _ => panic!("expected Return(Number(2))") }
    }
}
