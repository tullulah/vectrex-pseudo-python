...
DCR_DELTA_TABLE:\n\
    ...
    if w.contains("SHOW_LEVEL_RUNTIME") {
        ...
        out.push_str("    \n");
        out.push_str("    ; Read object counts (CRITICAL: CLRB first to avoid garbage in high byte)\n");
        out.push_str("    CLRB             ; Clear B register to ensure clean 8-bit loads\n");
        out.push_str("    LDA ,X+          ; A = bgCount\n");
        out.push_str("    STA >LEVEL_BG_COUNT\n");
        out.push_str("    LDA ,X+          ; A = gameplayCount\n");
        out.push_str("    STA >LEVEL_GP_COUNT\n");
        out.push_str("    LDA ,X+          ; A = fgCount\n");
        out.push_str("    STA >LEVEL_FG_COUNT\n");
        out.push_str("    \n");
        ...
        out.push_str("    BEQ SLR_OBJ_DONE ; Exit if zero (no more objects)\n");
