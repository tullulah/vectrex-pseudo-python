...
DCR_DELTA_TABLE:\n\
    ...
    // ============================================================================
    // LEVEL SYSTEM RUNTIME HELPERS (.vplay format support)
    // ============================================================================
    // These helpers manage level data from .vplay assets at runtime:
    // - LOAD_LEVEL_RUNTIME: Copy level header from ROM to RAM
    // - GET_OBJECT_COUNT_RUNTIME: Get number of objects in a layer
    // - GET_OBJECT_PTR_RUNTIME: Get pointer to specific object data
    // - GET_LEVEL_BOUNDS_RUNTIME: Extract world bounds from level header
    //
    // Level ROM structure (generated by levelres.rs):
    //   _LEVELNAME_LEVEL:
    //     FDB xMin, xMax, yMin, yMax       ; Bounds (8 bytes)
    //     FDB timeLimit, targetScore       ; Metadata (4 bytes)
    //     FCB bg_count, gameplay_count, fg_count  ; Object counts (3 bytes)
    //     FDB _LEVELNAME_BG_OBJECTS        ; Pointer to background layer
    //     FDB _LEVELNAME_GAMEPLAY_OBJECTS  ; Pointer to gameplay layer
    //     FDB _LEVELNAME_FG_OBJECTS        ; Pointer to foreground layer
    //
    // RAM variables (allocated in backend/m6809/mod.rs):
    //   LEVEL_PTR (2 bytes) - Current level header pointer
    //   LEVEL_BG_PTR, LEVEL_GAMEPLAY_PTR, LEVEL_FG_PTR (6 bytes) - Layer pointers
    // ============================================================================
    ...
    if w.contains("GET_OBJECT_COUNT_RUNTIME") {
        out.push_str(
            "; GET_OBJECT_COUNT_RUNTIME - Get number of objects in a layer\n\
    ...
    if w.contains("GET_OBJECT_PTR_RUNTIME") {
        out.push_str(
    ...
    if w.contains("GET_LEVEL_BOUNDS_RUNTIME") {
        out.push_str(
            "; GET_LEVEL_BOUNDS_RUNTIME - Extract world bounds from level header\n\
            ; Input: None (uses LEVEL_PTR)\n\
            ; Output: RESULT+0 = xMin (16-bit)\n\
            ;         RESULT+2 = xMax (16-bit)\n\
            ;         RESULT+4 = yMin (16-bit)\n\
            ;         RESULT+6 = yMax (16-bit)\n\
            ; Modifies: D, X\n\
            GET_LEVEL_BOUNDS_RUNTIME:\n\
                LDX >LEVEL_PTR          ; Load level header pointer\n\
                BEQ .GLB_ERROR          ; Error if level not loaded\n\
                \n\
                ; Bounds are first 8 bytes of header: xMin, xMax, yMin, yMax\n\
                LDD ,X++                ; Read xMin (auto-increment by 2)\n\
                STD RESULT+0            ; Store xMin\n\
                \n\
                LDD ,X++                ; Read xMax\n\
                STD RESULT+2            ; Store xMax\n\
                \n\
                LDD ,X++                ; Read yMin\n\
                STD RESULT+4            ; Store yMin\n\
                \n\
                LDD ,X                  ; Read yMax (no increment)\n\
                STD RESULT+6            ; Store yMax\n\
                RTS\n\
                \n\
            .GLB_ERROR:\n\
