...
MAIN:
    ...
    STD RESULT
    ; ===== GET_LEVEL_WIDTH builtin =====
    CLR RESULT             ; Clear high byte
    LDA LEVEL_WIDTH        ; Load width (byte)
    STA RESULT+1           ; Store in low byte
    LDD RESULT
    STD VAR_WIDTH
    ; ===== GET_LEVEL_HEIGHT builtin =====
    CLR RESULT             ; Clear high byte
    LDA LEVEL_HEIGHT       ; Load height (byte)
    STA RESULT+1           ; Store in low byte
    LDD RESULT
    STD VAR_HEIGHT
    LDD #-50
...
LOOP:
    ...
    LDD #0
    STD RESULT
    ; ===== GET_LEVEL_TILE builtin =====
    ; Get tile at (0, 0)
    LDA #0                 ; Y coordinate
    LDB LEVEL_WIDTH        ; Multiply by width
    MUL                    ; D = y * width
    ADDD #0               ; Add X coordinate
    ADDD #2                ; Skip width/height header
    ADDD LEVEL_PTR         ; Add base pointer
    TFR D,X                ; X = tile address
    CLR RESULT             ; Clear high byte
