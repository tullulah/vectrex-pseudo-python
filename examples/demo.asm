...
INIT_START: LDS #Vec_Default_Stk
    ...
    TFR A,DP
    BRA ENTRY_LOOP
ENTRY_LOOP: JSR VECTREX_WAIT_RECAL
    JSR VECTREX_SILENCE
    JSR VECTREX_BLINK_INT
    JSR >Reset0Ref
    JSR MAIN
...
VECTREX_BLINK_INT:
    ...
    STA BLINK_STATE
    BEQ BLINK_LOW
BLINK_HIGH: LDA #$5F ; high intensity
    BRA BLINK_SET
BLINK_LOW:  LDA #$10 ; low intensity (dim)
...
VECTREX_DEBUG_DRAW:
    ...
    RTS
DEBUG_DRAW_LIST:
    FCB $80,$40 ; short horizontal line (dy=0, dx=64)
VECTREX_SILENCE:
...
VECTREX_FRAME_BEGIN:
    ...
    RTS
VECTREX_WAIT_RECAL:
    ; Fast wait: simulate BIOS Wait_Recal (no VIA spin)
    LDA #$D0
    TFR A,DP
    LDA FAST_WAIT_HIT
    INCA
