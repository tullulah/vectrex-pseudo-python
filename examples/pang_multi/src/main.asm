...
.IF_END:
    ...
    ; TODO: Expr Logic { op: Or, left: Logic { op: Or, left: Logic { op: Or, left: Compare { op: Eq, left: Index { target: Ident(IdentInfo { name: "joystick1_state", source_line: 110, col: 4 }), index: Number(2) }, right: Number(1) }, right: Compare { op: Eq, left: Index { target: Ident(IdentInfo { name: "joystick1_state", source_line: 110, col: 31 }), index: Number(3) }, right: Number(1) } }, right: Compare { op: Eq, left: Index { target: Ident(IdentInfo { name: "joystick1_state", source_line: 110, col: 58 }), index: Number(4) }, right: Number(1) } }, right: Compare { op: Eq, left: Index { target: Ident(IdentInfo { name: "joystick1_state", source_line: 110, col: 85 }), index: Number(5) }, right: Number(1) } }
    LDD #0
    STD RESULT
    LDD RESULT
    LBEQ .IF_ELSE
...
draw_game_level:
    ...
    LDD RESULT
    STD VAR_joy_x
    ; TODO: Expr Logic { op: Or, left: Compare { op: Lt, left: Ident(IdentInfo { name: "joy_x", source_line: 306, col: 3 }), right: Number(-20) }, right: Compare { op: Gt, left: Ident(IdentInfo { name: "joy_x", source_line: 306, col: 18 }), right: Number(20) } }
    LDD #0
    STD RESULT
    LDD RESULT
    LBEQ .IF_ELSE
...
.IF_END:
    ...
    ; TODO: Expr Logic { op: Or, left: Compare { op: Lt, left: Ident(IdentInfo { name: "joy_x", source_line: 347, col: 3 }), right: Number(-80) }, right: Compare { op: Gt, left: Ident(IdentInfo { name: "joy_x", source_line: 347, col: 18 }), right: Number(80) } }
    LDD #0
    STD RESULT
    LDD RESULT
    LBEQ .IF_ELSE
...
VAR_anim_threshold   EQU $C880+$14   ; User variable: anim_threshold (2 bytes)
VAR_enemy_vy         EQU $C880+$16   ; User variable: enemy_vy (2 bytes)
VAR_count            EQU $C880+$18   ; User variable: count (2 bytes)
VAR_location_glow_intensity EQU $C880+$1A   ; User variable: location_glow_intensity (2 bytes)
VAR_abs_joy          EQU $C880+$1C   ; User variable: abs_joy (2 bytes)
VAR_prev_joy_x       EQU $C880+$1E   ; User variable: prev_joy_x (2 bytes)
VAR_hook_active      EQU $C880+$20   ; User variable: hook_active (2 bytes)
VAR_move_speed       EQU $C880+$22   ; User variable: move_speed (2 bytes)
VAR_location_x_coords EQU $C880+$24   ; User variable: location_x_coords (2 bytes)
VAR_joystick1_state  EQU $C880+$26   ; User variable: joystick1_state (2 bytes)
VAR_enemy_y          EQU $C880+$28   ; User variable: enemy_y (2 bytes)
VAR_level_enemy_speed EQU $C880+$2A   ; User variable: level_enemy_speed (2 bytes)
VAR_player_facing    EQU $C880+$2C   ; User variable: player_facing (2 bytes)
VAR_enemy_vx         EQU $C880+$2E   ; User variable: enemy_vx (2 bytes)
VAR_player_x         EQU $C880+$30   ; User variable: player_x (2 bytes)
VAR_STATE_GAME       EQU $C880+$32   ; User variable: STATE_GAME (2 bytes)
VAR_end_y            EQU $C880+$34   ; User variable: end_y (2 bytes)
VAR_player_y         EQU $C880+$36   ; User variable: player_y (2 bytes)
VAR_BOUNCE_DAMPING   EQU $C880+$38   ; User variable: BOUNCE_DAMPING (2 bytes)
VAR_end_x            EQU $C880+$3A   ; User variable: end_x (2 bytes)
VAR_MIN_BOUNCE_VY    EQU $C880+$3C   ; User variable: MIN_BOUNCE_VY (2 bytes)
VAR_enemy_active     EQU $C880+$3E   ; User variable: enemy_active (2 bytes)
VAR_screen           EQU $C880+$40   ; User variable: screen (2 bytes)
VAR_hook_max_y       EQU $C880+$42   ; User variable: hook_max_y (2 bytes)
VAR_mirror_mode      EQU $C880+$44   ; User variable: mirror_mode (2 bytes)
VAR_GROUND_Y         EQU $C880+$46   ; User variable: GROUND_Y (2 bytes)
VAR_num_locations    EQU $C880+$48   ; User variable: num_locations (2 bytes)
VAR_current_music    EQU $C880+$4A   ; User variable: current_music (2 bytes)
VAR_location_glow_direction EQU $C880+$4C   ; User variable: location_glow_direction (2 bytes)
VAR_current_location EQU $C880+$4E   ; User variable: current_location (2 bytes)
VAR_hook_gun_y       EQU $C880+$50   ; User variable: hook_gun_y (2 bytes)
VAR_title_state      EQU $C880+$52   ; User variable: title_state (2 bytes)
VAR_hook_gun_x       EQU $C880+$54   ; User variable: hook_gun_x (2 bytes)
VAR_STATE_TITLE      EQU $C880+$56   ; User variable: STATE_TITLE (2 bytes)
VAR_player_anim_counter EQU $C880+$58   ; User variable: player_anim_counter (2 bytes)
VAR_countdown_active EQU $C880+$5A   ; User variable: countdown_active (2 bytes)
VAR_enemy_size       EQU $C880+$5C   ; User variable: enemy_size (2 bytes)
VAR_GRAVITY          EQU $C880+$5E   ; User variable: GRAVITY (2 bytes)
VAR_i                EQU $C880+$60   ; User variable: i (2 bytes)
VAR_hook_init_y      EQU $C880+$62   ; User variable: hook_init_y (2 bytes)
VAR_hook_y           EQU $C880+$64   ; User variable: hook_y (2 bytes)
VAR_loc_x            EQU $C880+$66   ; User variable: loc_x (2 bytes)
VAR_active_count     EQU $C880+$68   ; User variable: active_count (2 bytes)
VAR_level_enemy_count EQU $C880+$6A   ; User variable: level_enemy_count (2 bytes)
VAR_player_anim_speed EQU $C880+$6C   ; User variable: player_anim_speed (2 bytes)
VAR_MAX_ENEMIES      EQU $C880+$6E   ; User variable: MAX_ENEMIES (2 bytes)
VAR_speed            EQU $C880+$70   ; User variable: speed (2 bytes)
VAR_location_names   EQU $C880+$72   ; User variable: location_names (2 bytes)
VAR_hook_x           EQU $C880+$74   ; User variable: hook_x (2 bytes)
VAR_joystick_poll_counter EQU $C880+$76   ; User variable: joystick_poll_counter (2 bytes)
VAR_prev_joy_y       EQU $C880+$78   ; User variable: prev_joy_y (2 bytes)
VAR_enemy_x          EQU $C880+$7A   ; User variable: enemy_x (2 bytes)
VAR_start_x          EQU $C880+$7C   ; User variable: start_x (2 bytes)
VAR_start_y          EQU $C880+$7E   ; User variable: start_y (2 bytes)
VAR_player_anim_frame EQU $C880+$80   ; User variable: player_anim_frame (2 bytes)
VAR_joy_y            EQU $C880+$82   ; User variable: joy_y (2 bytes)
VAR_countdown_timer  EQU $C880+$84   ; User variable: countdown_timer (2 bytes)
...
VAR_loc_y            EQU $C880+$8A   ; User variable: loc_y (2 bytes)
VAR_title_intensity  EQU $C880+$8C   ; User variable: title_intensity (2 bytes)
VAR_STATE_MAP        EQU $C880+$8E   ; User variable: STATE_MAP (2 bytes)
