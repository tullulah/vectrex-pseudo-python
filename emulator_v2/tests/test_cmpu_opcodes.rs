...
fn test_cmpu_immediate_0x11_83() {
    ...
    cpu.memory_bus().borrow_mut().write(0xC801, 0x83); // CMPU immediate
...
fn test_cmpu_direct_0x11_93() {
    ...
    cpu.memory_bus().borrow_mut().write(0xC800, 0x11); // Page 2 prefix
    cpu.memory_bus().borrow_mut().write(0xC801, 0x93); // CMPU direct
...
fn test_cmpu_extended_0x11_B3() {
    ...
    cpu.memory_bus().borrow_mut().write(0xC800, 0x11); // Page 2 prefix
    cpu.memory_bus().borrow_mut().write(0xC801, 0xB3); // CMPU extended
...
fn test_cmpu_indexed_0x11_A3() {
    ...
    cpu.memory_bus().borrow_mut().write(0xC800, 0x11); // Page 2 prefix
    cpu.memory_bus().borrow_mut().write(0xC801, 0xA3); // CMPU indexed
...
fn test_cmpu_stack_pointer_values() {
    ...
    cpu.memory_bus().borrow_mut().write(0xC800, 0x11); // Page 2 prefix
    cpu.memory_bus().borrow_mut().write(0xC801, 0x83); // CMPU immediate
    cpu.memory_bus().borrow_mut().write(0xC802, 0xD0); // High byte
