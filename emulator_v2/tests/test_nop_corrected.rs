//! Tests for NOP (No Operation) instruction - opcode 0x12
//! 
//! This test module verifies the implementation of the NOP instruction,
//! ensuring it performs no operation while correctly advancing the PC
//! and consuming the proper number of cycles.
...
fn test_nop_sequence() {
    ...
    let memory_bus = cpu.memory_bus().clone();
    memory_bus.borrow_mut().write(0xC800, 0x12); // NOP
    memory_bus.borrow_mut().write(0xC801, 0x12); // NOP
    memory_bus.borrow_mut().write(0xC802, 0x12); // NOP
    memory_bus.borrow_mut().write(0xC803, 0x86); // LDA #immediate (for visible change)
    memory_bus.borrow_mut().write(0xC804, 0x99); // Immediate value
    ...
    // Execute third NOP
    cpu.execute_instruction(false, false);
    assert_eq!(cpu.registers().pc, 0xC803, "PC should be at LDA instruction");
...
fn test_nop_memory_isolation() {
    ...
    let memory_bus = cpu.memory_bus().clone();
    memory_bus.borrow_mut().write(0xCA00, 0xAA);
    memory_bus.borrow_mut().write(0xCA01, 0xBB);
    memory_bus.borrow_mut().write(0xCA02, 0xCC);
    
    // Set up NOP instruction
    memory_bus.borrow_mut().write(0xC800, 0x12);
    ...
    assert_eq!(memory_bus.borrow().read(0xCA00), 0xAA, "Memory should not change");
    assert_eq!(memory_bus.borrow().read(0xCA01), 0xBB, "Memory should not change");
    assert_eq!(memory_bus.borrow().read(0xCA02), 0xCC, "Memory should not change");
}
...
fn test_nop_timing_consistency() {
    ...
    assert_eq!(cycles1, 2, "First NOP should take 2 cycles");
    
    let cycles2 = cpu.execute_instruction(false, false);
    assert_eq!(cycles2, 2, "Second NOP should take 2 cycles");
    
    let cycles3 = cpu.execute_instruction(false, false);
    assert_eq!(cycles3, 2, "Third NOP should take 2 cycles");
}
