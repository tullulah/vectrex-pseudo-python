//! Test for LDY extended (0x10BE) - Load Y register extended
//! C++ Original: OpLD<1, 0xBE>(Y);
...
#[test]
fn test_ldy_extended_0x10be() {
    let mut cpu = create_test_cpu();
    
    // 1. Setup inicial - Y register con valor conocido
    cpu.registers.y = 0x0000;
    
    let extended_addr: u16 = RAM_START + 0x200; // Dirección extendida en área de RAM válida
    let test_value: u16 = 0x5678;
    
    // Escribir el valor en la dirección extendida
    cpu.memory_bus().borrow_mut().write(extended_addr, (test_value >> 8) as u8);
    cpu.memory_bus().borrow_mut().write(extended_addr + 1, (test_value & 0xFF) as u8);
    
    // 2. Escribir opcode y operandos en memoria
    cpu.memory_bus().borrow_mut().write(RAM_START + 0x100, 0x10); // Page 1 prefix
    cpu.memory_bus().borrow_mut().write(RAM_START + 0x101, 0xBE); // LDY extended opcode
    cpu.memory_bus().borrow_mut().write(RAM_START + 0x102, (extended_addr >> 8) as u8); // Extended address high
    cpu.memory_bus().borrow_mut().write(RAM_START + 0x103, (extended_addr & 0xFF) as u8); // Extended address low
    ...
    assert_eq!(cycles, 7); // LDY extended should take 7 cycles according to table
}