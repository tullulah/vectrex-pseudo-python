// Tests para opcode 0x05 (Reserved)
...
fn test_reserved_0x05_panics() {
    let (mut cpu, memory) = setup_emulator();
    
    // Escribir opcode 0x05 en memoria
    memory.borrow_mut().write(RAM_START, 0x05);
...
#[test]
fn test_reserved_0x05_not_in_valid_opcodes() {
    // Verificar que 0x05 no está en la lista de opcodes válidos MC6809
    ...
    // 0x05 NO debe estar en la lista
    assert!(!valid_opcodes.contains(&0x05), 
            "0x05 is reserved and should not be in valid opcodes");
}
