// Tests para opcode 0x0B (Reserved)
...
fn test_reserved_0x0b_panics() {
    let (mut cpu, memory) = setup_emulator();
    
    // Escribir opcode 0x0B en memoria
    memory.borrow_mut().write(RAM_START, 0x0B);
...
#[test]
fn test_reserved_0x0b_not_in_valid_opcodes() {
    // Verificar que 0x0B no está en la lista de opcodes válidos MC6809
    ...
    // 0x0B NO debe estar en la lista
    assert!(!valid_opcodes.contains(&0x0B), 
            "0x0B is reserved and should not be in valid opcodes");
}
