// Tests para opcode 0x01 (Reserved)
...
fn test_reserved_0x01_panics() {
    let (mut cpu, memory) = setup_emulator();
    
    // Escribir opcode 0x01 en memoria
    memory.borrow_mut().write(RAM_START, 0x01);
...
#[test]
fn test_reserved_0x01_not_in_valid_opcodes() {
    // Verificar que 0x01 no está en la lista de opcodes válidos MC6809
    ...
    // 0x01 NO debe estar en la lista
    assert!(!valid_opcodes.contains(&0x01), 
            "0x01 is reserved and should not be in valid opcodes");
}
