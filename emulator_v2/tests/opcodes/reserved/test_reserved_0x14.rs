// Tests para opcode 0x14 (Reserved)
...
fn test_reserved_0x14_panics() {
    let (mut cpu, memory) = setup_emulator();
    
    // Escribir opcode 0x14 en memoria
    memory.borrow_mut().write(RAM_START, 0x14);
...
#[test]
fn test_reserved_0x14_not_in_valid_opcodes() {
    // Verificar que 0x14 no está en la lista de opcodes válidos MC6809
    ...
    // 0x14 NO debe estar en la lista
    assert!(!valid_opcodes.contains(&0x14), 
            "0x14 is reserved and should not be in valid opcodes");
}
