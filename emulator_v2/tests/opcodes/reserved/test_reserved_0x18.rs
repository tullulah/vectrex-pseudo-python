// Tests para opcode 0x18 (Reserved)
...
fn test_reserved_0x18_panics() {
    let (mut cpu, memory) = setup_emulator();
    
    // Escribir opcode 0x18 en memoria
    memory.borrow_mut().write(RAM_START, 0x18);
...
#[test]
fn test_reserved_0x18_not_in_valid_opcodes() {
    // Verificar que 0x18 no está en la lista de opcodes válidos MC6809
    ...
    // 0x18 NO debe estar en la lista
    assert!(!valid_opcodes.contains(&0x18), 
            "0x18 is reserved and should not be in valid opcodes");
}
