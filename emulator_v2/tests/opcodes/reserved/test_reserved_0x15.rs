// Tests para opcode 0x15 (Reserved)
...
fn test_reserved_0x15_panics() {
    let (mut cpu, memory) = setup_emulator();
    
    // Escribir opcode 0x15 en memoria
    memory.borrow_mut().write(RAM_START, 0x15);
...
#[test]
fn test_reserved_0x15_not_in_valid_opcodes() {
    // Verificar que 0x15 no está en la lista de opcodes válidos MC6809
    ...
    // 0x15 NO debe estar en la lista
    assert!(!valid_opcodes.contains(&0x15), 
            "0x15 is reserved and should not be in valid opcodes");
}
