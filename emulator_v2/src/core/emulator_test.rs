...
mod tests {
    ...
    fn test_emulator_creation() {
        ...
        assert_eq!(memory_bus.borrow().device_count(), 0); // Should have no devices connected yet
    ...
    fn test_memory_devices_connected() {
        ...
        for i in 1..device_count {
            let prev = memory_bus.borrow().device_info(i-1).unwrap();
            let curr = memory_bus.borrow().device_info(i).unwrap();
        ...
        ram.borrow_mut().write(0x0000, 0x42);
        let value = ram.borrow().read(0x0000);
