...
use crate::core::{Emulator, engine_types::{Input, RenderContext, AudioContext}};
...
impl VectrexEmulator {
    ...
    pub fn run_frame(&mut self, cycles: u64) {
        ...
        // Clear vectors from previous frame
        self.vectors_draw.clear();
        self.vector_draw_cnt = 0;
        ...
        while cycles_remaining > 0 && self.running {
            let cpu_cycles = self.emulator.execute_instruction(
                &self.input,
    ...
    fn update_input(&mut self) {
        ...
        // Update Input structure (TODO: Add joystick fields to Input)
        // self.input.joystick_x = x_axis;
        // self.input.joystick_y = y_axis;
        
        // Update button state
        // JSVecx Pattern: shadow_snd_regs14 bits for buttons (A=0x01, S=0x02, D=0x04, F=0x08)
        // self.input.button1 = self.button1_held;
        // self.input.button2 = self.button2_held;
        // self.input.button3 = self.button3_held;
        // self.input.button4 = self.button4_held;
        
        let _ = (x_axis, y_axis); // Suppress unused warnings until Input is extended
    }
    ...
    pub fn get_vectors_json(&self) -> String {
        serde_json::to_string(&self.vectors_draw[..self.vector_draw_cnt])
    ...
    pub fn get_registers(&mut self) -> String {
        ...
        format!(
            ...
            regs.pc, regs.a, regs.b, regs.x, regs.y, regs.u, regs.s, regs.dp, regs.cc.to_u8()
        )
    }
    ...
    pub fn step(&mut self) {
        ...
        let cycles = self.emulator.execute_instruction(
            &self.input,
            &mut self.render_context,
            &mut self.audio_context
        );
    ...
    pub fn get_pc_history(&self) -> String {
        let history: Vec<String> = self.pc_history.iter()
            .map(|(pc, opcode)| format!("{{\"pc\":\"0x{:04X}\",\"opcode\":\"0x{:02X}\"}}", pc, opcode))
            .collect();
        format!("[{}]", history.join(","))
    }
}
