...
fn run_one(cpu: &mut CPU, op: u8) -> u64 {
    cpu.pc = 0x0100;
    cpu.bus.mem[0x0100] = op; cpu.bus.mem[0x0100] = op; // mirror into bus backing mem
...
fn direct_addressing_baseline_cycles() {
    ...
    cpu.bus.mem[0x0020] = 0; cpu.bus.mem[0x0020] = 0;
    cpu.pc = 0x0100; cpu.a = 0x12;
    cpu.bus.mem[0x0100] = 0x97; cpu.bus.mem[0x0100] = 0x97; // STA direct
    cpu.bus.mem[0x0101] = 0x20; cpu.bus.mem[0x0101] = 0x20; // direct addr
...
fn branch_short_taken_adds_one_cycle() {
    ...
    cpu.pc = 0x0100;
    cpu.bus.mem[0x0100] = 0x20; cpu.bus.mem[0x0100] = 0x20; // BRA
    cpu.bus.mem[0x0101] = 0x02; cpu.bus.mem[0x0101] = 0x02; // skip two bytes
    let before = cpu.cycles; cpu.step(); let delta = cpu.cycles - before;
...
fn jsr_extended_cycles() {
    ...
    cpu.pc = 0x0100;
    cpu.bus.mem[0x0100] = 0xBD; cpu.bus.mem[0x0100] = 0xBD;
    cpu.bus.mem[0x0101] = 0x02; cpu.bus.mem[0x0101] = 0x02; // high
    cpu.bus.mem[0x0102] = 0x00; cpu.bus.mem[0x0102] = 0x00; // low
    let before = cpu.cycles; cpu.step(); let delta = cpu.cycles - before;
