...
// BIOS-only startup trace test analogous to Rust call_stack_capture_demo
TEST(BiosStartup, EarlyInstructionTrace) {
    // Locate BIOS image relative to repository root (assumes run from project root or deeper)
    // Reuse existing filesystem helpers.
    fs::path biosPath = "ide/frontend/dist/bios.bin";
    if (!fs::exists(biosPath)) {
        // Try one level up (in case tests run from vectrexy/ subdir)
        if (fs::exists("../ide/frontend/dist/bios.bin")) biosPath = "../ide/frontend/dist/bios.bin";
    }
    ASSERT_TRUE(fs::exists(biosPath)) << "BIOS not found at " << biosPath;

    // Build a MemoryBus, load BIOS, then reset CPU.
    MemoryBus memoryBus; // default constructs with zeroed memory
    {
        std::ifstream fin(biosPath, std::ios::binary);
        ASSERT_TRUE(fin.good());
        std::vector<uint8_t> buf( (std::istreambuf_iterator<char>(fin)), std::istreambuf_iterator<char>() );
        ASSERT_TRUE(buf.size()==4096 || buf.size()==8192) << "Unexpected BIOS size: " << buf.size();
        // Placement: 4K -> 0xF000, 8K -> 0xE000 (mirrors Rust load_bios logic)
        uint16_t base = (buf.size()==4096)? 0xF000 : 0xE000;
        for (size_t i=0;i<buf.size();++i) memoryBus.Write(base + (uint16_t)i, buf[i]);
    }

    Cpu cpu; // exposed wrapper around CpuImpl
    cpu.Init(memoryBus); // ensure bus attached
    cpu.Reset();

    uint16_t startPc = cpu.Registers().PC; // after reset
    std::cout << "[bios] start PC=$" << std::hex << std::uppercase << startPc << std::dec << "\n";
    ASSERT_GE(startPc, 0xE000); // basic sanity

    // Step a small number of instructions and capture their PCs/opcodes
    struct StepInfo { uint16_t pc; uint8_t opcode; };
    std::vector<StepInfo> steps;
    steps.reserve(40);
    for (int i=0;i<40;++i) {
        uint16_t pcBefore = cpu.Registers().PC;
        uint8_t opcode = memoryBus.Read(pcBefore);
        steps.push_back({pcBefore, opcode});
        cpu.ExecuteInstruction(true,true); // allow IRQ/FIRQ evaluation flags (placeholders)
    }

    std::cout << "[bios] first instructions:" << std::hex << std::uppercase << "\n";
    for (size_t i=0;i<steps.size() && i<25; ++i) {
        std::cout << std::setw(4) << std::setfill('0') << steps[i].pc << ": "
                  << std::setw(2) << (int)steps[i].opcode << "\n";
    }

    // Heuristic: ensure we did NOT immediately run into unmapped 0x00 NEG loop unless BIOS actually has it.
    // If first meaningful (non LDS) instruction after initial sequence is 0x00 and repeats many times, flag.
    int negCount = 0; for (auto& s : steps) if (s.opcode==0x00) ++negCount; 
    EXPECT_LT(negCount, 10) << "Unexpected early run of 0x00 bytes (possible wrong reset vector or memory load)";
}
