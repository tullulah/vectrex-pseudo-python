...
namespace {
    ...
    template<typename T>
    int narrow_cast_int(T v) {
        if (v > static_cast<T>(std::numeric_limits<int>::max())) return std::numeric_limits<int>::max();
        if (v < static_cast<T>(std::numeric_limits<int>::min())) return std::numeric_limits<int>::min();
...
void DapDebugger::InitDap() {
    ...
    m_session->registerHandler([&](const dap::StackTraceRequest& request)
        ...
        for (auto iter = frames.rbegin(); iter != frames.rend(); ++iter, ++i) {
            ...
            frame.column = 1;
            frame.id = narrow_cast_int(frames.size() - i - 1); // Store callstack index
    ...
    m_session->registerHandler(
        [&](const dap::ScopesRequest& request) -> dap::ResponseOrError<dap::ScopesResponse> {
            ...
            int callStackIndex = narrow_cast_int(request.frameId);
            ...
            if (auto function = m_debugSymbols.GetFunctionByAddress(frame.frameAddress)) {
                ...
                dapScope.variablesReference =
                    VariableRef{VariableRef::Type::CallStackIndex, narrow_cast_int(callStackIndex)}.AsInt();
    ...
    m_session->registerHandler(
        [&](const dap::VariablesRequest& request) -> dap::ResponseOrError<dap::VariablesResponse> {
            ...
            VariableRef variableRef{narrow_cast_int(request.variablesReference)};
